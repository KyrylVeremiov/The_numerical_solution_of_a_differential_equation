...
plt.title("O(h)_numerical_solution")
plt.savefig("O(h)_numerical_solution.png")
plt.show()
...
plt.title("O(h)_original function")
plt.savefig("O(h)_original_function.png")
plt.show()

fig = plt.figure()
ax = fig.add_subplot(projection='3d')
ax.plot_surface(X, Y, abs(u_t(X, Y) - u), cmap='plasma')
plt.title("O(h)_residues")
plt.savefig("O(h)_residues.png")
plt.show()

pd.DataFrame(u).to_csv("O(h)_numerical_solution.csv")
